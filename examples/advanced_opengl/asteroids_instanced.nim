...
for i in 0..<amount:
  ...
  var displacement = rand((2*offset*100).int).float32 / 100.0'f32 - offset
  let x = sin(angle) * radius + displacement
  displacement = rand((2*offset*100).int).float32 / 100.0'f32 - offset
  ...
  let scale = rand(20).float32 / 100.0'f32 + 0.05'f32
  model = scale(model,vec3(scale))

  # 3. rotation: add random rotation around a (semi)randomly picked rotation axis vector
  let rotAngle = rand(360).float32
...
let _ = genBindBuffer(GL_ARRAY_BUFFER) 
bufferData(GL_ARRAY_BUFFER, amount * Mat4f.sizeof(), modelMatrices[0].addr,GL_STATIC_DRAW)
...
for mesh in rock.meshes:   
  ...
  vertexAttribPointer(3,4,cGL_FLOAT,false,Mat4f.sizeof(),0)
  enableVertexAttribArray(4)
  vertexAttribPointer(4,4,cGL_FLOAT,false,Mat4f.sizeof(),Vec4f.sizeof())
  enableVertexAttribArray(5)
  vertexAttribPointer(5,4,cGL_FLOAT,false,Mat4f.sizeof(),2*Vec4f.sizeof())
  enableVertexAttribArray(6)
  vertexAttribPointer(6,4,cGL_FLOAT,false,Mat4f.sizeof(),3*Vec4f.sizeof())
...
while run:
  ...
  bindTexture(GL_TEXTURE_2D,rock.textures_loaded[0].id)
  for mesh in rock.meshes:
    bindVertexArray(mesh.VAO)
    drawElementsInstanced(GL_TRIANGLES,mesh.indices.len,GL_UNSIGNED_INT,amount)
    unBindVertexArray()
